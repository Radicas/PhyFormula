...
struct MaterialParams
{
    // 热力学参数（表1）
    double C11 = 106.8e9, C12 = 60.4e9, C44 = 28.3e9;
    double C111 = -1076e9, C112 = -315e9, C123 = 36e9, C144 = -23e9, C166 = -340e9, C456 = -30e9;  // 新增三阶弹性常数
    double rho0 = 2700.0;                                                                          // kg/m³
    double mu0 = 26.1e9;                                                                           // Pa
    double mu_p = 6.52e-2;                                                                         // 无量纲
    double mu_T = -6.16e-4;                                                                        // K⁻¹
...
struct CrystalState
{
    Matrix3d F_e = Matrix3d::Identity();
    Matrix3d F_p = Matrix3d::Identity();
    Matrix3d E_e_prev = Matrix3d::Zero();  // 新增：存储上一步弹性应变
    double T = 300.0;                      // K
    double rho_m;                          // 可动位错密度 (m⁻²)
    double rho_im;                         // 不可动位错密度 (m⁻²)
    std::vector<double> gamma_dots;
    std::vector<double> tau_alpha;  // 新增：存储解析剪切应力
...
class CrystalPlasticityModel
{
   public:
    ...
    void updateState(CrystalState& state, const Matrix3d& F_total, double dt)
    {
        Matrix3d F_e_trial = F_total * state.F_p.inverse();
        Matrix3d E_e = 0.5 * (F_e_trial.transpose() * F_e_trial - Matrix3d::Identity());
        Matrix3d dE_e = E_e - state.E_e_prev;  // 弹性应变增量

        Matrix3d S = computeSecondPKStress(E_e, dE_e, state.T);        // 修改参数
        state.tau_alpha = computeResolvedShearStresses(S, F_e_trial);  // 存储tau

        state.gamma_dots = computeSlipRates(state);
        updateDislocationDensity(state, dt);
        updateTemperature(state, S, dE_e, dt);  // 使用dE_e
        updatePlasticDeformation(state, dt);

        state.E_e_prev = E_e;  // 更新弹性应变历史
    }
    Matrix3d computeSecondPKStress(const Matrix3d& E_e, const Matrix3d& dE_e, double T)
    {
        // 二阶项
        Matrix3d S = mp_.C11 * E_e + mp_.C12 * E_e.trace() * Matrix3d::Identity();

        // 三阶项（论文公式1.10）- 修复括号匹配
        S(0, 0) += 0.5 * (mp_.C111 * E_e(0, 0) * E_e(0, 0) + mp_.C112 * (E_e(1, 1) + E_e(2, 2)) * E_e(0, 0));
        S(1, 1) += 0.5 * (mp_.C111 * E_e(1, 1) * E_e(1, 1) + mp_.C112 * (E_e(0, 0) + E_e(2, 2)) * E_e(1, 1));
        S(2, 2) += 0.5 * (mp_.C111 * E_e(2, 2) * E_e(2, 2) + mp_.C112 * (E_e(0, 0) + E_e(1, 1)) * E_e(2, 2));

        // 剪切分量
        S(0, 1) += mp_.C144 * E_e(0, 1) * E_e(0, 1);
        S(1, 2) += mp_.C144 * E_e(1, 2) * E_e(1, 2);
        S(0, 2) += mp_.C144 * E_e(0, 2) * E_e(0, 2);

        // 热弹性项
        Matrix3d thermal_term = (mp_.alpha_v * (mp_.C11 + 2 * mp_.C12) / 3.0) * (T - 300.0) * Matrix3d::Identity();
        S -= thermal_term;
   ...
   private:
    ...
    std::vector<double> computeSlipRates(CrystalState& state)
    {
        ...
        for (size_t i = 0; i < slip_systems_.size(); ++i)
        {
            ...
            double v_r = 1e-6;  // 初始猜测
            double L = 1.0 / sqrt(state.rho_im);
            for (int iter = 0; iter < 100; ++iter)
            {
                double v_new = (std::abs(tau) - mp_.tau_ath) * mp_.b / B_T * (1 - pow(v_r / C_t, 2));
                if (std::abs(v_new - v_r) < 1e-6 || v_r > 0.99 * C_t)
                    break;
                v_r = v_new;
            }
            v_r = std::min(v_r, 0.99 * C_t);  // 限速
    ...
    void updateDislocationDensity(CrystalState& state, double dt)
    {
        ...
        for (double tau : state.tau_alpha)
        {
            ...
            rho_het_total += mp_.alpha_het * (mp_.m_het + 1) * pow(x, mp_.m_het);
        }
        ...
        state.rho_m += rho_het_total + rho_mult - rho_ann - rho_trap + rho_depin;
        state.rho_im += rho_trap - rho_depin;
        state.rho_m = std::max(state.rho_m, 0.0);
        state.rho_im = std::max(state.rho_im, 0.0);
    }

    void updateTemperature(CrystalState& state, const Matrix3d& S, const Matrix3d& dE_e, double dt)
    {
        double W_p = 0.0;
        for (size_t i = 0; i < slip_systems_.size(); ++i)
            W_p += S.cwiseProduct(slip_systems_[i].second * slip_systems_[i].first.transpose()).sum() *
                   state.gamma_dots[i] * dt;

        // 修正：使用弹性应变率（dE_e/dt）
        double term2 = state.T * mp_.alpha_v * (dE_e / dt).trace() / (mp_.rho0 * mp_.cv);

        state.T += (mp_.beta_TQ * W_p) / (mp_.rho0 * mp_.cv) - term2;
    }

    void updatePlasticDeformation(CrystalState& state, double dt) { /* 与原代码相同 */ }
...
TEST(CrystalPlasticity, StressOvershoot)
{
    ...
    for (int i = 0; i < steps; ++i)
    {
        ...
        // 正确计算冯·米塞斯应力（基于偏应力）
        Matrix3d sigma = (1.0 / state.F_e.determinant()) * state.F_e *
                         model.computeSecondPKStress(state.E_e_prev, Matrix3d::Zero(), state.T) * state.F_e.transpose();
        Matrix3d sigma_dev = sigma - (sigma.trace() / 3.0) * Matrix3d::Identity();
        double von_mises = sqrt(1.5 * sigma_dev.cwiseProduct(sigma_dev).sum());
    ...
    auto max_it = std::max_element(stress_history.begin(), stress_history.end());
    EXPECT_GT(*max_it, stress_history.back())
        << "应力超调现象未出现，最大应力：" << *max_it << "，最终应力：" << stress_history.back();
}